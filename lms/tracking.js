...
function callAPI(eventname, eventtype, eventdata) {
    ...
    var seUrl = parameters.seUrl;
    var eventsource = 'Storyline';

    // Update variables used by in-course JS.
    window.playerHost = seUrl.replace('/tracking', '');
    window.mainWindowURLParams = window.location.search.substr(1);

    var eventurl = seUrl
        + '/api/sessionevents.php?sessionid=' + encodeURI(sessionId)
        + '&eventname=' + encodeURI(eventname)
        + '&eventtype=' + encodeURI(eventtype)
        + '&eventsource=' + encodeURI(eventsource)
        + '&eventdata=' + encodeURI(eventdata)
        + '&sesskey=' + encodeURI(sesskey)
        + '&scoid=' + encodeURI(scoId)
        + '&setid=' + encodeURI(setid);
    ...
    xhttp.onreadystatechange = function () {
        ...
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            try {
                window.parent.parent.location.href = seUrl + '/html/index.php?error_code=' + eventresponse.Code;
            } catch (e) {
...
function loadTraliantState() {
    ...
    var seUrl = parameters.seUrl;
    var apiUrl = seUrl.substring(0, seUrl.indexOf('/tracking'));
    ...
    xhttp.onreadystatechange = function () {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            if (data.current_part_events > 3) {
                window.parent.postMessage('Traliant.trackingDetected', '*');
            }
...
function checkDownloadSpeed() {
    if (window.location.origin != 'https://content.traliant.com') {
        return;
    }
    ...
    http.onload = function (e) {
        if (http.status == 200) {
            ...
            var rate = (e.loaded / 1048576) / (et / 1000); // MB/s
            var data = { transferred: e.loaded, elapsedTime: et + ' ms', rate: rate.toFixed(2) + ' MB/s' };
            callAPI('SpeedTest', data.elapsedTime, JSON.stringify(data, null, 2));
        }
    }

    var startTime = (new Date()).getTime();
    var url = 'https://content.traliant.com/external/speedtest/500kb.txt?' + startTime;
...
function isTraliantLMS() {
    return !!(getUrlParameter('seurl') || getFlashUrlParameter('seurl'));
}
...
function getFlashUrlParameter(name) {
    return decodeURIComponent((new RegExp('[?|&]' + name + '=' + '([^&;]+?)(&|#|;|$)').exec(document.referrer)
        || [null, ''])[1].replace(/\+/g, '%20'))
        || null;
}
...
function parseUrlParameters() {
    ...
    var seUrl = getUrlParameter('seurl');
    ...
    if (!seUrl) {
        ...
        seUrl = getFlashUrlParameter('seurl');
    }
